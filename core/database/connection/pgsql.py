...
class PgsqlConnectionManager:
    ...
    def start(self) -> None:
        ...
        self._monitor_thread = threading.Thread(
            target=self._monitor_loop, daemon=True, name="pgsql-monitor"
        )
    ...
    def get_connection(self) -> PgConnection | None:
        """返回当前活跃的 PostgreSQL 连接，若尚未连接则返回 None。"""
        with self._lock:
            return self._conn
    ...
    def _monitor_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            if not self._is_alive():
                custom_log("WARNING", "PostgreSQL 连接已断开，正在尝试重连...")
                while not self._stop_event.is_set():
                    if self._connect():
                        retry_interval = _INITIAL_RETRY_INTERVAL
                        break
                    custom_log(
                        "WARNING",
                        f"PostgreSQL 重连失败，{retry_interval} 秒后重试...",
                    )
